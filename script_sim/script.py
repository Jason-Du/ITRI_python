...
def spice_data_anlyz(input_lib=""):
    ...
    for line_idx,line in enumerate(spice_txt):
        ...
        if par is not None and last_par is None and par.group(1)!="LEVEL": # last_par is None 條件是將 {//////*(1+tol)} 給予 if last_par is not None: # 程式碼去處理
...
last_pars,mod_par_dict=spice_data_anlyz(input_lib=r"C:\Users\user\Documents\LTspiceXVII\file\LIB\r8002cnd3.lib")
...
def ajst_lib_content(mod_par_dict={},now_par_idxs=[],input_lib=""):
    ...
    for now_par_idx in now_par_idxs:        ##改動的內容 ={*(1+tol)}}
        ...
        string_list[now_par_info["idx"]] = "+ {}={{{}*(1+tol)}}\n".format(now_par_info['par_name'],now_par_info['par_val'])
    ...
    spice_txt.close()
    output_file_str = ''.join([str(list(now_par_idx.keys())[0]) + "_" + str(now_par_idx["par_name"]) for now_par_idx in now_par_idxs])
    output_file="./lib/r8002cnd3_"+output_file_str+".lib".format(list(now_par_idx.keys())[0],now_par_idx["par_name"])
...
if __name__ == '__main__':
    ...
    for f_idx,f in enumerate(files):
        print("{} IS UNDER SIMULATION...................{} % ".format(f+1,float(f_idx/len(files))*100))
        ...
        LTC.run()
        LTC.wait_completion()
        shutil.copyfile(r"C:\Users\user\Documents\LTspiceXVII\file\Montecarlo\monte_SiC_1.log", "./result/"+str(log_name.group(1))+".log")

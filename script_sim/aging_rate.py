...
if dR_AGING:
    ...
    axes2.set_title("Power MOSFET DeltaR Variation T=483~523K Vgs=10V Freq=1kHz VDS=4V ", fontsize=20)
    axes2.set_xlabel("Aging Time (min)", fontsize=15)
    axes2.set_ylabel("deltaR variation(%)", fontsize=15)
...
if Temp_time:
    ...
    if Single:
        ...
        axes.set_xlabel("Test Run No.", fontsize=15)
...
if Temp_dR:
    ...
    for  time_label,rdson_label,MOSFET_rdson,MOSFET_TEMP in zip(df_rdson.columns[0::2],df_rdson.columns[1::2],MOSFET_rdson_ll,MOSFET_TEMP_ll):
        ...
        df2=df2.replace([0.01],0).set_index("run")
